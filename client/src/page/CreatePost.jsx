...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://dall-e-yiew.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch('https://dall-e-yiew.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    <section className="max-w-full mx-auto bg-[#1b1a19] bg-opacity-0 p-20 rounded-3xl">
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        ...
        <div className="mt-10">
          <button
            type="submit"
            className="mt-3 text-white bg-[#fc731e] font-medium rounded-md text-sm w-full sm:w-auto px-5 py-2.5 text-center"
          >
      ...
      <img src={brain} alt='brain' className="object-contain scale-75"/>
      </div>
